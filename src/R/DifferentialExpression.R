...
"line_plot" <- function(dds=dds,vst=vst,gene_id=gene_id){
    ...
    p <- ggplot(bind_cols(as.data.frame(colData(dds)),
                ...
                aes(x=Stage,y=value,col=Tissue,group=Tissue)) +
...
#' B8_vs_B4 - in FMG!
B8_vs_B4 <- extract_results(dds=dds,vst=vst,contrast="Stage_B8_vs_B4",
                            default_dir = here("analysis/DE"),
                            default_prefix = "FMG_B8_vs_B4",
                            labels=dds$Stage,
                            sample_sel = dds$Tissue == "FMG")

#' ZE_vs_FMG - at B4!
ZE_vs_FMG <- extract_results(dds=dds,vst=vst,contrast="Tissue_ZE_vs_FMG",
                            default_dir = here("analysis/DE"),
                            default_prefix = "B4_ZE_vs_FMG",
                            labels=dds$Tissue,
                            sample_sel = dds$Stage == "B4")

#' Specific effect in ZE at B8
B8_ZE <- extract_results(dds=dds,vst=vst,contrast="StageB8.TissueZE",
                             default_dir = here("analysis/DE"),
                             default_prefix = "B8_ZE",
                         labels=paste(dds$Tissue,dds$Stage))

dev.null <- lapply(B8_ZE$all,line_plot,dds=dds,vst=vst)

#' B8_vs_B4 - in ZE?
ZE_B8_vs_B4 <- extract_results(dds=dds,vst=vst,contrast=c(0,1,0,1),
                            default_dir = here("analysis/DE"),
                            default_prefix = "ZE_B8_vs_B4",
                            labels=dds$Stage,
                            sample_sel = dds$Tissue == "ZE")

#' ZE_vs_FMG - at B8?
B8_ZE_vs_FMG <- extract_results(dds=dds,vst=vst,contrast=c(0,0,1,1),
                             default_dir = here("analysis/DE"),
                             default_prefix = "B8_ZE_vs_FMG",
                             labels=dds$Tissue,
                             sample_sel = dds$Stage == "B8")
...
#' called `res.list`
#' ```
res.list <- list(FMG=B8_vs_B4,
         ZE=ZE_B8_vs_B4,
         intx=B8_ZE)

dev.null <- lapply(intersect(B8_vs_B4$all,ZE_B8_vs_B4$all),
...
#' ```
background <- rownames(vst)[featureSelect(vst,factor(paste(dds$Tissue,dds$Stage)),
                                          exp=1)]

stopifnot(all(unlist(sapply(res.list,"[","all"))%in%background))

background <- sub("\\.1","",background)
